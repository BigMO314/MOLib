...
namespace MOLib {
	namespace PID {
		...
		class MagEncLoop : public BaseLoop {
		public:
			virtual ~MagEncLoop() {}
			MagEncLoop(MOLib::Sensors::MagEncoder *enc_Source)
			:BaseLoop(enc_Source), enc_Source(enc_Source) {}
			MagEncLoop(double P, double I, double D, MOLib::Sensors::MagEncoder *enc_Source)
			:BaseLoop(P, I, D, enc_Source), enc_Source(enc_Source) {}
			virtual void ResetSource() override { enc_Source->Reset(); }
		private:
			MOLib::Sensors::MagEncoder* enc_Source;
		};
